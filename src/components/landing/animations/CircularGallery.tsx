...
function lerp(p1: any, p2: any, t: any) {
    return p1 + (p2 - p1) * t;
}
...
function createTextTexture(gl: any, text: any, font = 'bold 30px monospace', color = 'black') {
...
class Media {
    ...
    createShader() {
        ...
        // Add Anisotropic filtering for sharpness
    ...
    createMesh() {
        ...
        // Store reference to media data on the mesh for click detection
        this.plane.userData = { image: this.image, text: this.text };
    }
    createTitle() {
...
class App {
    ...
    onTouchMove(e: any) {
        ...
        const bounds = this.renderer.gl.canvas.getBoundingClientRect();
        this.mouse.set(
            2.0 * (x - bounds.left) / this.renderer.width - 1.0,
            2.0 * (1.0 - (y - bounds.top) / this.renderer.height) - 1.0
        );
    ...
    onTouchUp(e: any) {
        ...
        const dt = performance.now() - this.clickStart;
    ...
    handleClick() {
        ...
        const meshes = this.medias.map((m: any) => m.plane);
        const hits = this.raycast.intersectBounds(meshes);

        if (hits && hits.length) {
            const hit = hits[0];
            // Access user data from the hit mesh
            if (hit.userData) {
                this.onItemClick(hit.userData);
            }
        }
    }
    onWheel(e: any) {
